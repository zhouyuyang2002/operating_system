...
mem_init(void)
{
	...
	// pde_t * useless = (pde_t *) boot_alloc(PGSIZE)
	// Magic... If this line is not added in, then &kern_pgdir
	// is located in [kern_pgdir, kern_pgdir + PGSIZE), which means that it
	// will be filled with 0 in line 152, cause errors !!
	// I guess the reason is that there is something wrong with linker...
...
page_init(void)
{
	...
	for (i = 0; i < npages; i++) {
		if (i == 0){
			...
			//case 4), Memory Allocated in boot_alloc()
			pages[i].pp_link = NULL;
			pages[i].pp_ref = 1;
		}
		else if (i * PGSIZE == MPENTRY_PADDR){
			//case 5), additional memory usage in lab5.
			pages[i].pp_link = NULL;
			pages[i].pp_ref = 1;
		}
		else{
			//case 2), case 4), Free Memory:
			pages[i].pp_ref = 0;
...
pgdir_walk(pde_t *pgdir, const void *va, int create)
{
	...
	if ((*dict_ptr) & PTE_P){
		// present for use, page exist
		return (pte_t*)(KADDR(PTE_ADDR(*dict_ptr)) + PTX(va) * sizeof(pte_t));
	}
	else{
		//not presenr for use, need to create;
		if (create == false)
			return NULL;
...
page_lookup(pde_t *pgdir, void *va, pte_t **pte_store)
{
	...
	if ((*pte_itr) & PTE_P){
		if (pte_store != NULL)
			*pte_store = pte_itr;
		return (struct PageInfo*)pa2page(PTE_ADDR(*pte_itr));
	}
	else
		//No access to 
		return NULL;
