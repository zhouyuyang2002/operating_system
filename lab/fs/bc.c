...
bc_pgfault(struct UTrapframe *utf)
{
	...
	if (r < 0) panic("in bc_pgfault, sys_page_alloc: %e", r);
	r = ide_read(blockno * (BLKSIZE / SECTSIZE), addr, BLKSIZE / SECTSIZE);
	if (r < 0) panic("in bc_pgfault, ide_read: %e", r);
...
flush_block(void *addr)
{
	...
	if (r < 0) panic("in flush_block, ide_write: %e", r);
	int perm = uvpt[PGNUM(addr)] & PTE_SYSCALL;
	r = sys_page_map(0, addr, 0, addr, perm);
	if (r < 0) panic("in flush block, sys_page_map: %e", r);
