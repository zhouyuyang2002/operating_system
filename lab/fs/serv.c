...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	r = openfile_lookup(envid, req->req_fileid, &ofile);
	if (r < 0) return r;

	r = file_read(ofile->o_file, ret->ret_buf, 
				MIN(req->req_n, BLKSIZE), ofile->o_fd->fd_offset);
	if (r < 0) return r;
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	r = file_write(ofile->o_file, req->req_buf,
				   MIN(BLKSIZE, req->req_n), ofile->o_fd->fd_offset);
	if (r < 0) return r;
	ofile->o_fd->fd_offset += r;
	return r;
